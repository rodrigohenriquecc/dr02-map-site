...
function initUI(){
  ...
  menu.querySelectorAll("button").forEach(btn=>{
    btn.onclick=()=>{
      const m=btn.dataset.mode; menu.style.display="none";
...
function processLineExcel(file){
  ...
  reader.onload=e=>{
    ...
    rows.forEach(rw=>{
      ...
      // extrair feature como LineString (ou flatten MultiLineString)
      let feat=null;
      ...
      } else if(gj.geometry && gj.geometry.type==="LineString"){
        feat=gj;
      }
      if(!feat){
        console.warn("sem feature linear:",key);
        return;
      }
...
function kmlToGeoJSON(xmlStr){
  ...
  Array.from(dom.getElementsByTagName("Placemark")).forEach(pm=>{
    ...
    if(coords.length>1) feats.push({type:"Feature",
