...
const HEAT_SHEET   =
  "https://docs.google.com/spreadsheets/d/1W61josvM1UanGOSUurj1qSZTvpL4ovzf/export?format=csv";
const LINE_SHEET   =
  "https://docs.google.com/spreadsheets/d/14dAXHzNvDQb8gFgOZCeTnOOcgWEpqaoA/export?format=csv";

// ── 4) Load metaRod via CSV ─────────────────────────────────
Papa.parse(
...
// ── 5) Carrega RC + KMZ + Online Sheets ────────────────────
async function carregarData(){
  ...
  // 5.4) Load online sheets
  loadPointsOnline();
  loadHeatOnline();
  loadLinesOnline();

  // bind botão recarregar
  document.getElementById("btnCSV").onclick=()=>carregarData();

  zoomGlobal();
}

// ── 6) Load & Process Online CSVs ───────────────────────────
function addPontoCsv(d){
...
function loadPointsOnline(){
  ...
  Papa.parse(POINTS_SHEET,{
    download:true, header:true, skipEmptyLines:true,
    complete:({data})=>{
      data.forEach(addPontoCsv);
      zoomGlobal();
    },
    error:err=>console.warn("erro pontos online:",err)
  });
}
...
function loadHeatOnline(){
  ...
  Papa.parse(HEAT_SHEET,{
    ...
    complete:({data})=>{
      ...
      heatLayer=L.heatLayer(pts,{radius:25,blur:15})
                  .addTo(mapa);
      zoomGlobal();
    },
    error:err=>console.warn("erro heat online:",err)
  });
}
...
function loadLinesOnline(){
  ...
  Papa.parse(LINE_SHEET,{
      ...
      data.forEach(rw=>{
        ...
        const gj=seg.toGeoJSON(),
              feat= gj.features.find(f=>f.geometry.type==="LineString")
                   || (()=>{
                       const ml=gj.features
                         .find(f=>f.geometry.type==="MultiLineString");
                       return ml?{
                         type:"Feature",
                         geometry:{
                           type:"LineString",
    ...
    error:err=>console.warn("erro lines online:",err)
  });
}
