...
Papa.parse(META_URL,{
  ...
  complete:({data})=>{
    data.forEach(r=>{
      const [kmIni,kmFim]=[r.kmIni,r.kmFim].map(s=>parseFloat(s.replace(",",".")).toFixed(3));
      const [iniLat,iniLon]=r.LatLonIni.split(",").map(Number);
      const [fimLat,fimLon]=r.LatLonFim.split(",").map(Number);
      metaRod[r.id]={kmIni:parseFloat(kmIni),iniLat,iniLon,
                     kmFim:parseFloat(kmFim),fimLat,fimLon};
    });
    carregarData();
  }
});
...
async function carregarData(){
  ...
  // RC shapefiles (estilo sem preenchimento)
  for(const p of [
  ...
  // KMZ rodovias — agora filtrando Points
  for(const id of Object.keys(metaRod)){
    try{
      ...
      rodLayers[id]=L.geoJSON(geo,{
        pane:"rodoviasPane",
        // ⬇️ só desenha LineString / MultiLineString
        filter:f=>f.geometry && (
...
/* … resto do código (loadPoints, loadHeat, loadLines) permanece igual … */
