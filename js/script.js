/* global L, JSZip, shp, turf, XLSX, firebase */
...
function zoomGlobal(){
  ...
  const b = L.featureGroup(g).getBounds();
...
// ─── 2) Carrega metaRod do Google Sheets CSV ───────────────────
const SHEET_CSV_URL =
...
// ─── 3) Carrega RC e KMZ Rodovias ───────────────────────────────
async function carregarData(){
  // RC shapefiles
  const zips=["data/RC_2.1.zip","data/RC_2.2.zip","data/RC_2.4.zip",
  ...
  for(const id of Object.keys(metaRod)){
    ...
    try{
      ...
      if(!kml){ console.warn("Sem KML:",url); continue; }
...
function initUI(){
  ...
  document.getElementById("csvPointsInput").onchange = e=>{
    e.target.files[0] && processPointsExcel(e.target.files[0]);
  };
  document.getElementById("csvHeatInput").onchange = e=>{
    e.target.files[0] && processHeatExcel(e.target.files[0]);
  };
  document.getElementById("csvLineInput").onchange = e=>{
    e.target.files[0] && processLineExcel(e.target.files[0]);
  };
  // KM locator
...
function processPointsExcel(file){
  ...
  reader.onload = e=>{
    ...
    if(pontosLayer.getLayers().length)
      mapa.fitBounds(pontosLayer.getBounds());
  };
  reader.readAsArrayBuffer(file);
}

// ─── 6) Mapa de Calor (Excel) ───────────────────────────────────
function processHeatExcel(file){
  ...
  reader.onload=e=>{
    ...
    rows.forEach(r=>{
      ...
      const m=metaRod[key];
      const km0=parseFloat(String(r["Km Inicial"]).replace(",",".")),
