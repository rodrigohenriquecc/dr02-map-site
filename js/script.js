...
/* CONFIGURAÇÕES                                              */
/* ---------------------------------------------------------- */
const LIMITE_METROS = 2000;          // novo segmento se pular > 2 km
...
let camadaRecorte    = null;         // polilinha vermelha (filtro)
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  criarPainelRodovia();              // novo cartão flutuante
...
async function carregarRodovias(map){
  ...
  const raw=XLSX.utils.sheet_to_json(
    XLSX.read(buf,{type:'array'}).Sheets['Sheet1']||XLSX.read(buf,{type:'array'}).Sheets[0],
...
function criarPainelRodovia(){
  ...
  ['change','input','input'].forEach((ev,i)=>{
    ['selRod','kmIni','kmFim'][i]&&document.getElementById(
      ['selRod','kmIni','kmFim'][i]).addEventListener(ev,aplicarFiltro);
  });
}
...
function aplicarFiltro(){
  ...
  if(rod&&(!isNaN(ki)||!isNaN(kf))){
    ...
    if(pts.length){
      camadaRecorte=L.polyline(pts.sort((a,b)=>a.km-b.km)
        .map(p=>[p.lat,p.lon]),{color:'#d00',weight:5,opacity:1}).addTo(mapa);
