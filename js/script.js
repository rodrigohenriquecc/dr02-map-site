...
/* ────────── META-DADOS Km ────────── */
const metaRod = {};   // id → {kmIni,kmFim, iniLat,iniLon, …}
...
KMZ_FILES.forEach(async file=>{
  try{
    ...
    /* marcador início/fim se fornecido no JSON */
...
function localizarKm(){
  ...
  L.circleMarker([lat,lon],dotOpt)
    .bindPopup(`<b>${rod}</b><br>KM ${km.toFixed(3)}`).addTo(mapa)
    .openPopup();
