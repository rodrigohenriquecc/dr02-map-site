...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  /* painel (criado logo aqui!) ------------------------------ */
  const mobile = window.innerWidth <= 600;
  layerControl = L.control.layers(null, {}, {
    position: 'topright', collapsed: mobile }).addTo(mapa);

  /* dados --------------------------------------------------- */
  await carregarRCs();          // adiciona contornos + rótulos RC
  await carregarRodovias();     // adiciona contornos + rótulos rodovia

  aplicarMascaraRC();           // máscara fora da área RC
  criarPainelKm();              // cartão “≡” (filtro Km)

  /* zoom geral ---------------------------------------------- */
  const tudo = L.featureGroup(Object.values({ ...rcOverlays, ...rodOverlays }));
...
async function carregarRCs() {
  ...
  for (const { n, f } of arquivos) {
    try {
      ...
      layerControl.addOverlay(rcLabels[n], n + ' (rótulo)');
...
function criarPainelKm() {
  ...
  const opts = Object.keys(rodoviaDados).sort()
...
function filtrarKm() {
  ...
  if (rod && (!isNaN(ki) || !isNaN(kf))) {
    const pts = rodoviaDados[rod].filter(p =>
      (isNaN(ki) || p.km >= ki) && (isNaN(kf) || p.km <= kf));
    if (pts.length) {
      linhaRecorte = L.polyline(
