...
const rodLabels   = [];                    // para evitar sobreposição
...
function updateRodLabels(){
  ...
  rodLabels.forEach(m=>{
    ...
    let ok=true;
    for(const u of used){ if(Math.abs(p.x-u.x)<50&&Math.abs(p.y-u.y)<50){ok=false;break;} }
...
Papa.parse(SHEET_CSV_URL,{download:true,header:true,skipEmptyLines:true,
  complete:({data})=>{
    data.forEach(r=>{
...
async function carregarData(){
  ...
  for(const id of Object.keys(metaRod)){
    ...
    }catch(e){console.error('KMZ',url,e);}
  }
...
function initUI(){
  ...
  document.getElementById('csvPointsInput').onchange=e=>e.target.files[0]&&processPointsExcel(e.target.files[0]);
  document.getElementById('csvHeatInput').onchange  =e=>e.target.files[0]&&processHeatExcel(e.target.files[0]);
...
function processPointsExcel(file){
  ...
  r.onload=e=>{
    ...
    rows.forEach(d=>{
      ...
      if(!seg){const mk=Object.keys(rodLayers).find(k=>k.endsWith('_'+d.Rodovia));if(mk)seg=rodLayers[mk],key=mk;}
...
function processHeatExcel(file){
  ...
  r.onload=e=>{
    ...
    rows.forEach(rw=>{
      ...
      if(!seg){const mk=Object.keys(rodLayers).find(k=>k.endsWith('_'+rw.Rodovia));if(mk)seg=rodLayers[mk],key=mk;}
      const m=metaRod[key],km0=parseFloat(String(rw['Km Inicial']).replace(',','.')),km1=parseFloat(String(rw['Km Final']).replace(',','.'));
      ...
      for(let i=0;i<=samples;i++){const p=turf.along(slice,(Ls*i)/samples,{units:'kilometers'});pts.push([p.geometry.coordinates[1],p.geometry.coordinates[0],1]);}
    });
...
// 8) Firestore estrelar
function carregarFirestore(){
  col.get()
    .then(snap=>{ snap.forEach(doc=>{/*ignored*/}); zoomGlobal(); })
...
async function salvarFirestore(){
  ...
  try{
    ...
    const del=db.batch(); snap.forEach(d=>del.delete(d.ref)); await del.commit();
    const set=db.batch(); pts.forEach((d,i)=>set.set(col.doc(String(i)),d)); await set.commit();
